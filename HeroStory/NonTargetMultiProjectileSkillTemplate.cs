...
[CreateAssetMenu(menuName = "Skill/Non Target Multi Projectile", order = 999)]
public class NonTargetMultiProjectileSkillTemplate : SkillTemplate
{
    [Header("Skill Damage")]
    ...
    [Header("Projectile")]
    //public bool sequentialAttack = false;
    //public float sequentialAttackGapTime = 0.0f;

    public ProjectileSkillEffect projectile;
    public EffectMount[] projtileEffectMount; //투사체가 나가는 위치 ex) 총구 칼 끝
    public LevelBasedInt projectileCount = new LevelBasedInt { baseValue = 1 };
    public LevelBasedFloat startAngle = new LevelBasedFloat { baseValue = 0 };
    public LevelBasedFloat endAngle = new LevelBasedFloat { baseValue = 0 };

    [Header("Visual Effect")]
    public bool useCurrentProjectileMount = false;
    ...
    {
        if(!useCurrentProjectileMount) SpawnVisualEffect(caster, skillLevel);
        if (projectile != null)
        {
                float gap = Mathf.Abs((endAngle.Get(skillLevel) - startAngle.Get(skillLevel)) / (float)(projectileCount.Get(skillLevel) - 1));
                for (int i = 0; i < projectileCount.Get(skillLevel); i++)
                {
                    float angle = startAngle.Get(skillLevel) + (gap * i);
                    GameObject go = Instantiate(projectile.gameObject, caster.GetEffectMount(projtileEffectMount[i % projtileEffectMount.Length]).position, caster.GetEffectMount(projtileEffectMount[i % projtileEffectMount.Length]).rotation);
                    if (useCurrentProjectileMount) SpawnVisualEffect(caster, skillLevel, projtileEffectMount[i % projtileEffectMount.Length]);
                    ...
                    effect.targetPos = Extensions.RotatePosition(caster.transform.position, caster.targetPos, angle);
                    ...
                    //Debug.Log(i+"번 : gap : "+gap+", count : "+projectileCount.baseValue+ ", angle : "+angle+ ", effect rotation : " + effect.targetPos);
                }
        }
    ...
    public void SpawnVisualEffect(Entity caster, int skillLevel, EffectMount mount)
    {
        if (visualEffect_Invoke != null)
        {
            GameObject go = Instantiate(visualEffect_Invoke.gameObject, caster.GetEffectMount(mount).position, caster.GetEffectMount(mount).rotation);
            var _effect = go.GetComponent<VisualSkillEffect>();
            if (_effect.followEffectMount) go.transform.parent = caster.GetEffectMount(mount);
            if (effectMount == EffectMount.BothHands)
            {
                GameObject goRight = Instantiate(visualEffect_Invoke.gameObject, caster.GetEffectMount(EffectMount.RightHand).position, caster.GetEffectMount(EffectMount.RightHand).rotation);
                var _effectRight = goRight.GetComponent<VisualSkillEffect>();
